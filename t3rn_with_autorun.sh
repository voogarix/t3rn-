...
# Save configuration to file (add this before the final run command)
...
cat <<EOF > ../$CONFIG_FILE
{
    "node_type": "$NODE_TYPE",
    "version": "$LATEST_TAG",
    "archive_name": "executor-linux-$LATEST_TAG.tar.gz",
    "parameters": {
        ...
        "rpc_endpoints": {
            ...
            "BSSP": "$RPC_ENDPOINTS_BSSP",
            "BLSS": "$RPC_ENDPOINTS_BLSS",
            "OPSP": "$RPC_ENDPOINTS_OPSP",
            "L1RN": "$RPC_ENDPOINTS_L1RN"
        },
        "alchemy_key": "${ALCHEMY_API_KEY:-none}"
    }
}
EOF
